...
func (w *Worker) processJob(ctx context.Context, tx repository.Tx, job domain.OutboxJob) error {
	...
	switch job.Kind {
	case "execution_request_submitted", "policy_incident_review":
		var marker string
		if len(job.Payload) > 0 {
			if err := json.Unmarshal(job.Payload, &marker); err != nil && job.Kind == "policy_incident_review" {
				return fmt.Errorf("decode policy_incident job: %w", err)
			}
		}
		return nil
	default:
