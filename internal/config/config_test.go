...
func TestValidateAggregatesProblems(t *testing.T) {
	cfg := Config{HTTPAddr: "", DatabasePath: "", BusinessTimezone: "Missing/Timezone", ApprovalTaskTTL: time.Second, SessionTTL: time.Minute, WorkerInterval: time.Millisecond, WorkerBatchSize: 0, ShutdownTimeout: time.Millisecond, LogLevel: "verbose"}
	err := cfg.Validate()
	if err == nil {
...
func TestValidateBoundaryValues(t *testing.T) {
	base := Config{HTTPAddr: ":8080", DatabasePath: "db", BusinessTimezone: "UTC", ApprovalTaskTTL: time.Minute, SessionTTL: 15 * time.Minute, WorkerInterval: 100 * time.Millisecond, WorkerBatchSize: 1, ShutdownTimeout: time.Second, LogLevel: "debug"}
...
func TestAcceptedLogLevels(t *testing.T) {
	for _, level := range []string{"debug", "info", "warn", "error"} {
		cfg := Config{HTTPAddr: ":8080", DatabasePath: "db", BusinessTimezone: "UTC", ApprovalTaskTTL: time.Minute, SessionTTL: time.Hour, WorkerInterval: time.Second, WorkerBatchSize: 10, ShutdownTimeout: time.Second, LogLevel: level}
